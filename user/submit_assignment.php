        ...
        <div class="content-wrapper">
            <div class="row">
                <div class="col-12 grid-margin">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title mb-4">Assignment | Submit Assignment <i class="menu-icon mdi mdi-setting"></i></h5>
...
<?php
    if(isset($_POST['submit'])){
        ...
        $term            = $_POST['term'];        
        $class           = $_SESSION['sd']['class'];

        $sql = "SELECT * FROM assignment_submit WHERE subject = '$subject' AND term = '$term' AND class = '$class' AND admno = '$admno' ";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            echo '
                <div class="alert alert-warning">
                  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                  <strong>Waning!</strong> '.$subject.' Assignment Submission  for '.$class.' in respect to '.$term.' already Exist
                </div>
            ';
        }else{
            $sql1 = "INSERT INTO assignment_submit(subject, class, term, file, admno) VALUES('$subject', '$class', '$term', '$file_name', '$admno')";
            if($conn->query($sql1)){

                $destination     = "file/assignmnet_submitted/".$file_name;
